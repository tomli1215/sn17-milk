...
class Trellis2ImageTo3DPipeline(Pipeline):
    ...
    def _expand_cond(self, cond: dict, num_samples: int) -> dict:
        """
        Expand conditioning tensors to match num_samples batch size.
        """
        if num_samples <= 1:
            return cond
        ...
        for key, value in cond.items():
            if isinstance(value, torch.Tensor):
                expanded[key] = value.expand(num_samples, *value.shape[1:]).contiguous()
            else:
                expanded[key] = value
        return expanded
    ...
    def decode_latent(
        ...
        if self.offload_decoders:
            self.models['shape_slat_decoder'].to('cpu')
            torch.cuda.empty_cache()

        tex_voxels = self.decode_tex_slat(tex_slat, subs)
        if self.offload_decoders:
            self.models['tex_slat_decoder'].to('cpu')
            torch.cuda.empty_cache()
    ...
    def run(
        ...
        # Expand conditioning to match num_samples batch size
        cond_512 = self._expand_cond(cond_512, num_samples)
        if cond_1024 is not None:
            cond_1024 = self._expand_cond(cond_1024, num_samples)
    ...
    def run_shape(
        ...
        """
        Run only the shape generation phase (sparse structure + shape SLat).
        Returns intermediate state needed for texture generation.

        Returns:
            dict with keys: shape_slat, cond, res, generator, pipeline_type, tex_slat_sampler_params
        """
        pipeline_type = pipeline_type or self.default_pipeline_type
        if pipeline_type == '512':
    ...
    def run_texture(
        ...
        """
        Run only the texture generation phase from pre-computed shape.

        Args:
            shape_result: Dict returned by run_shape().
            num_samples: If provided and less than batch size, slice shape_slat
                         to keep only that many candidates.

        Returns:
            List[MeshWithVoxel]: The generated meshes with textures.
        """
        shape_slat = shape_result["shape_slat"]
        cond = shape_result["cond"]
        res = shape_result["res"]
        generator = shape_result["generator"]
        pipeline_type = shape_result["pipeline_type"]
        tex_slat_sampler_params = shape_result["tex_slat_sampler_params"]

        # Reduce candidates if requested
        if num_samples is not None and num_samples < shape_slat.shape[0]:
            shape_slat = shape_slat[:num_samples]
            # Also slice conditioning to match
            cond = {
                k: v[:num_samples] if isinstance(v, torch.Tensor) and v.shape[0] > num_samples else v
                for k, v in cond.items()
            }

        tex_flow_model_key = {
            '512': 'tex_slat_flow_model_512',
            '1024': 'tex_slat_flow_model_1024',
            '1024_cascade': 'tex_slat_flow_model_1024',
            '1536_cascade': 'tex_slat_flow_model_1024',
        }[pipeline_type]

        tex_slat = self.sample_tex_slat(
            cond, self.models[tex_flow_model_key],
            shape_slat, tex_slat_sampler_params,
            generator=generator
        )

        if self.offload_decoders:
            self.models[tex_flow_model_key].to('cpu')
        torch.cuda.empty_cache()
        return self.decode_latent(shape_slat, tex_slat, res)
