...
async def generate(prompt_image_file: UploadFile = File(...), seed: int = Form(-1)) -> StreamingResponse:
    ...
    try:
        ...
        # Generate GLB from uploaded file (pass filename for naming candidate outputs)
        glb_bytes = await asyncio.wait_for(
            pipeline.generate_from_upload(
                await prompt_image_file.read(),
                seed,
                image_name=prompt_image_file.filename or None,
            ),
...
@app.post("/preview_edit")
async def preview_edit(prompt_image_file: UploadFile = File(...), seed: int = Form(-1)) -> JSONResponse:
    """
    Run Qwen edit + background removal only (no TRELLIS). Returns JSON with
    seed, image_edited_grid_base64, image_without_background_grid_base64,
    images_edited_base64, images_without_background_base64.
    """
    try:
        image_bytes = await prompt_image_file.read()
        result = await asyncio.wait_for(
            pipeline.preview_edit(image_bytes, seed),
            timeout=settings.api.timeout,
        )
        return JSONResponse(content=result)
    except asyncio.TimeoutError:
        logger.error("preview_edit timed out")
        raise HTTPException(status_code=408, detail="timeout") from None
    except Exception as exc:
        logger.exception("preview_edit failed: %s", exc)
        raise HTTPException(status_code=500, detail=str(exc)) from exc
