...
# Relative output_dir is resolved against pipeline_service/ so writes work even if cwd != pipeline_service
_pipeline_root = Path(__file__).parent.parent
_out = settings.output.output_dir
if not _out.is_absolute():
    _out = (_pipeline_root / _out).resolve()
settings.output = settings.output.model_copy(update={"output_dir": _out})
